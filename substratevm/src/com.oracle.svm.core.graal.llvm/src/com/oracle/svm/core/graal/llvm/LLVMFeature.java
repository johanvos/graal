...
class LLVMAMD64TargetSpecificFeature implements Feature {
    ...
    public void afterRegistration(AfterRegistrationAccess access) {
        ImageSingletons.add(TargetSpecific.class, new TargetSpecific() {
            ...
            /*
             * The return address is pushed to the stack just before each call, but is not part of the
             * stack frame of the callee. It is therefore not accounted for in either call frame.
             */
            @Override
            ...
            /*
             * The frame pointer is stored as the first element on the stack, just below the return address.
             */
            @Override
...
class LLVMAArch64TargetSpecificFeature implements Feature {
    ...
    public void afterRegistration(AfterRegistrationAccess access) {
        ImageSingletons.add(TargetSpecific.class, new TargetSpecific() {
            ...
            /*
             * The return address is not saved on the stack on ARM, so the stack frames have no space inbetween them.
             */
            @Override
            public int getCallFrameSeparation() {
                return 0;
